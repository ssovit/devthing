module.exports = (basedir) => {
	...
	function scssTask(name) {
		...
		if (!isLive) {
			var pipe1 = source.pipe(plugins.clone()).pipe(plugins.sourcemaps.write('.')).pipe(gulp.dest(dest));
			if (config.scss[name].noMinify === true) {
				return pipe1.pipe(plugins.livereload());
			}
			mergeStream.add(pipe1);
		}
		var pipe2 = source.pipe(plugins.clone()).pipe(plugins.rename({
			suffix: '.min',
	...
	function jsTask(name) {
		...
		var source = gulp.src(config.js[name].files, {
			//read: false,
		}).pipe(plugins.vinylNamed()).pipe(plugins.webpackStream({
	...
	Object.keys(config.scss).forEach(function(group) {
	...
	Object.keys(config.js).forEach(function(group) {
		if (group != 'defaultDest') {
	...
	Object.keys(config.assets).forEach(function(group) {
		if (group != 'defaultDest') {
			gulp.task('copy:' + group, function() {
				...
				return gulp.src(config.assets[group].files, {
					allowEmpty: true,
				}).pipe(gulp.dest(dest)).pipe(plugins.livereload());
			});
		}
	});
	...
	gulp.task('plugin:mainfile', function() {
		return gulp.src(config.plugin.file).pipe(plugins.replace(/(Plugin Name:\s?)(.*)/g, '$1' + config.plugin.title)).pipe(plugins.replace(/(Plugin URI:\s?)(.*)/g, '$1' + config.plugin.pluginUrl)).pipe(plugins.replace(/(Author:\s?)(.*)/g, '$1' + config.plugin.author)).pipe(plugins.replace(/(Author URI:\s?)(.*)/g, '$1' + config.plugin.authorUrl)).pipe(plugins.replace(/(Description:\s?)(.*)/g, '$1' + config.plugin.description)).pipe(plugins.replace(/(Version:\s?)(.*)/g, '$1' + config.plugin.version)).pipe(plugins.replace(/(Text Domain:\s?)(.*)/g, '$1' + config.plugin.textdomain)).pipe(plugins.replace(/(Requires PHP:\s?)(.*)/g, '$1' + config.plugin.php)).pipe(plugins.replace(/(License URI:\s?)(.*)/g, '$1' + config.plugin.licenseURI)).pipe(plugins.replace(/(License:\s?)(.*)/g, '$1' + config.plugin.license)).pipe(gulp.dest('./'));
	});
	gulp.task('plugin:plugin', function() {
		return gulp.src("includes/Plugin.php").pipe(plugins.replace(/(const VERSION\s?\=\s?\')(.*)(\')/g, '$1' + config.plugin.version + '$3')).pipe(plugins.replace(/(const TEXTDOMAIN\s?\=\s?\')(.*)(\')/g, '$1' + config.plugin.textdomain + '$3')).pipe(gulp.dest('includes'));
	});
	gulp.task("plugin:textdomain", function() {
		...
		return gulp.src(["**/*.php", '!vendor/', '!vendor/**', ], {
			base: "."
		}).pipe(plugins.replace(pattern, '$1$4$7' + config.plugin.textdomain + '$11')).pipe(gulp.dest('.'));
	});
